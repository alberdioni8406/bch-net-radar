...
module.exports = async (req, res) => {
  ...
  const [network, blocksResult, mempool, marketConsensus, blockConsensus] = await Promise.all([
    ...
    withGlobalTimeout(blockchainProviders.getMempoolDetail(), { unconfirmedTxCount: null, sizeBytes: null }),
  ...
  const payload = {
    ...
    mempool: (mempool.unconfirmedTxCount !== null || mempool.sizeBytes !== null)
      ? { ...mempool, status: 'fresh' }
      : { status: 'unavailable' },
