...
const haskoinMirror = createHaskoinProvider(
  'haskoin-mirror',
  process.env.HASKOIN_MIRROR_URL || 'https://api.blockchain.info/hashkoin-store/bch'
);
...
const blockchainProviders = {
  ...
  // Combines Haskoin's tx-count view with Blockchair's size/fee-rate view.
  // Each field is labeled with its own source — never blended into one
  // number that implies a single provider gave us all of it.
  async getMempoolDetail() {
    const [countResult, statsResult] = await Promise.all([
      this.getMempool(),
      withFallback([{ name: 'blockchair', fn: () => blockchair.getMempoolStats() }]),
    ]);

    return {
      unconfirmedTxCount: countResult.data?.count ?? statsResult.data?.txCount ?? null,
      countSource: countResult.data ? countResult.source : (statsResult.data ? statsResult.source : null),
      sizeBytes: statsResult.data?.sizeBytes ?? null,
      suggestedFeeSatPerByte: statsResult.data?.suggestedFeeSatPerByte ?? null,
      statsSource: statsResult.data ? statsResult.source : null,
    };
  },
